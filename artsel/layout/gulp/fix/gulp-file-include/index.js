...
const replaceOperator = require("../../../node_modules/gulp-file-include/lib/replace-operator");
const replaceFunction = require("../../../node_modules/gulp-file-include/lib/replace-function");
const replaceVariable = require("../../../node_modules/gulp-file-include/lib/replace-variable");
const concat = require("concat-stream");
const setIndent = require("../../../node_modules/gulp-file-include/lib/indent");
...
module.exports = function (opts) {
  ...
  if (opts.basepath !== "@file") {
    opts.basepath =
      opts.basepath === "@root" ? process.cwd() : path.resolve(opts.basepath);
  }
  ...
  function include(file, text, data, sourceFile = "") {
    var filebase =
      opts.basepath === "@file" ? path.dirname(file.path) : opts.basepath;
    ...
    function conditionalHandler(inst) {
      try {
        var condition = new Function(
          "var context = this; with (context) { return " + inst.args + "; }"
        ).call(data); // eslint-disable-line
      } catch (error) {
    ...
    function forHandler(inst) {
      ...
      var condition =
        'var context = this; with (context) { var result=""; ' +
        forLoop +
        " return result; }";
      try {
    ...
    function includeHandler(inst) {
      ...
      if (args) {
        ...
        // START FIX PATCH
        // Позволяет вставлять JSON файлы без использования @@Loop пример:
        // @@include('../blocks/content-card.html',{"file":"\/..\/components\/sections\/about-section.json"})
        ...
        if (args[3]) {
          ...
          if ("file" in includeJSON) {
            ...
            var jsonfile =
              file.base + datafile.replace(/^('|")/, "").replace(/('|")$/, "");
        ...
        recFile = include(
          recFile,
          includeContent,
          args[3] ? JSON5.parse(args[3]) : {},
          inst.sourceFile != "" ? inst.sourceFile : currentFilename
        );
